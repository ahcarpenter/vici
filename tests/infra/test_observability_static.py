...
class TestOBS01JaegerDeployments:
    ...
    def test_collector_exposes_otlp_grpc_port_4317(self) -> None:
        ...
        assert "_JAEGER_OTLP_GRPC_PORT = 4317" in self.source or "container_port=4317" in self.source, (
    ...
    def test_collector_exposes_otlp_http_port_4318(self) -> None:
        ...
        assert "_JAEGER_OTLP_HTTP_PORT = 4318" in self.source or "container_port=4318" in self.source, (
    ...
    def test_query_exposes_port_16686(self) -> None:
        ...
        assert "_JAEGER_QUERY_PORT = 16686" in self.source or "container_port=16686" in self.source, (
            "Query Deployment must expose port 16686"
        )
        assert "16686" in self.source

    def test_both_deployments_have_health_probes_on_port_13133_path_status(self) -> None:
        """Both collector and query must reference health port 13133 and path /status."""
        ...
        # Both deployments define liveness + readiness probes referencing the same health port
        health_port_count = self.source.count("13133")
        assert health_port_count >= 2, (
            f"Expected health port 13133 to appear in both deployments (>=2 uses), found {health_port_count}"
        )
    ...
    def test_both_deployments_have_resource_limits(self) -> None:
        ...
        assert self.source.count('"limits"') + self.source.count("'limits'") + self.source.count("limits=") >= 2, (
    ...
    def test_both_services_are_clusterip_type(self) -> None:
        ...
        clusterip_count = self.source.count('type="ClusterIP"') + self.source.count("type='ClusterIP'")
        assert clusterip_count >= 2, (
            f"Expected 2 ClusterIP services (collector + query), found {clusterip_count}"
        )
...
class TestOBS02ServiceMonitor:
    ...
    def test_servicemonitor_custom_resource_defined(self) -> None:
        ...
        assert 'kind="ServiceMonitor"' in self.source or "kind='ServiceMonitor'" in self.source, (
            "prometheus.py must define a ServiceMonitor CustomResource"
        )
        assert 'api_version="monitoring.coreos.com/v1"' in self.source or "api_version='monitoring.coreos.com/v1'" in self.source, (
    ...
    def test_servicemonitor_scrapes_metrics_path(self) -> None:
        ...
        assert '"path": "/metrics"' in self.source or "'path': '/metrics'" in self.source, (
...
class TestOBS03GrafanaStack:
    ...
    def test_kube_prometheus_stack_helm_release_defined(self) -> None:
        ...
        assert 'chart="kube-prometheus-stack"' in self.source or "chart='kube-prometheus-stack'" in self.source, (
    ...
    def test_jaeger_datasource_configured_in_additional_datasources(self) -> None:
        ...
        assert '"additionalDataSources"' in self.source or "'additionalDataSources'" in self.source, (
    ...
    def test_gke_autopilot_node_exporter_disabled(self) -> None:
        ...
        assert '"nodeExporter": {"enabled": False}' in self.source or "'nodeExporter': {'enabled': False}" in self.source, (
    ...
    def test_gke_autopilot_kube_controller_manager_disabled(self) -> None:
        ...
        assert '"kubeControllerManager": {"enabled": False}' in self.source or "'kubeControllerManager': {'enabled': False}" in self.source, (
    ...
    def test_gke_autopilot_kube_scheduler_disabled(self) -> None:
        ...
        assert '"kubeScheduler": {"enabled": False}' in self.source or "'kubeScheduler': {'enabled': False}" in self.source, (
    ...
    def test_gke_autopilot_kube_etcd_disabled(self) -> None:
        ...
        assert '"kubeEtcd": {"enabled": False}' in self.source or "'kubeEtcd': {'enabled': False}" in self.source, (
    ...
    def test_gke_autopilot_kube_proxy_disabled(self) -> None:
        ...
        assert '"kubeProxy": {"enabled": False}' in self.source or "'kubeProxy': {'enabled': False}" in self.source, (
    ...
    def test_gke_autopilot_core_dns_disabled(self) -> None:
        ...
        assert '"coreDns": {"enabled": False}' in self.source or "'coreDns': {'enabled': False}" in self.source, (
    ...
    def test_gke_autopilot_kube_dns_disabled(self) -> None:
        ...
        assert '"kubeDns": {"enabled": False}' in self.source or "'kubeDns': {'enabled': False}" in self.source, (
