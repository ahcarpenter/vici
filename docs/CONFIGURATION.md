...
For production on GKE, environment variables are managed via **GCP Secret Manager** and synced to Kubernetes Secrets by the External Secrets Operator (ESO). See [DEPLOYMENT.md](DEPLOYMENT.md) for details on the secrets pipeline.
